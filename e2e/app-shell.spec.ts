...
test('boots an empty app shell with menu actions gated until an image opens @smoke', async ({ page }) => {
  ...
  const gallerySymmetricalGardenItem = page.locator('#gallery-polyhaven-symmetrical-garden-02-1k-button');
  const galleryKaistItem = page.locator('#gallery-kaist-menu-button');
  const galleryKaistMenu = page.locator('#gallery-kaist-menu');
  const galleryKaistScene01Item = page.locator('#gallery-kaist-scene01-reflectance-button');
  const galleryKaistScene27Item = page.locator('#gallery-kaist-scene27-reflectance-button');
  const galleryKaistScene30Item = page.locator('#gallery-kaist-scene30-reflectance-button');
  ...
  await galleryPolyHavenItem.click();
  await expect(galleryPolyHavenMenu).toBeVisible();
  await expect(galleryArtistWorkshopItem).toBeVisible();
  await expect(galleryArtistWorkshopItem).toBeEnabled();
  await expect(galleryBrownPhotostudioItem).toBeVisible();
  await expect(galleryBrownPhotostudioItem).toBeEnabled();
  await expect(gallerySymmetricalGardenItem).toBeVisible();
  await expect(gallerySymmetricalGardenItem).toBeEnabled();
  await page.keyboard.press('Escape');
  await expect(galleryPolyHavenMenu).toBeHidden();
  await expect(galleryKaistItem).toBeVisible();
  await expect(galleryKaistItem).toBeEnabled();
  await expect(galleryKaistMenu).toBeHidden();
  await galleryKaistItem.click();
  await expect(galleryKaistMenu).toBeVisible();
  await expect(galleryKaistScene01Item).toBeVisible();
  await expect(galleryKaistScene01Item).toBeEnabled();
  await expect(galleryKaistScene27Item).toBeVisible();
  await expect(galleryKaistScene27Item).toBeEnabled();
  await expect(galleryKaistScene30Item).toBeVisible();
  await expect(galleryKaistScene30Item).toBeEnabled();
  await page.keyboard.press('Escape');
  await expect(galleryKaistMenu).toBeHidden();
  ...
  await expect(budgetModeInput.locator('option')).toHaveText(['Automatic', 'Fixed']);
...
test('opens the gallery demo image and keeps core display controls stable @smoke', async ({ page }) => {
  ...
  await expect(openedFileTooltip).toContainText('0.0 MB');
  await expect(openedFileTooltip).not.toContainText('compression');
