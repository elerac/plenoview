...
The renderer compiles separate ordinary-panorama and path-tracing programs on first use, with separate cached path-tracing variants
...
masking, mip averages, cache reuse/eviction, and polished silver reflection in
path tracing (including a one-bounce limit).
...
The test verifies both modes, exposure controls, compilation readiness,
...
ordinary panorama in 1.7 s, the HDR bake in 1.3 s, and path tracing in
